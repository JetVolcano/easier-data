...
class Point2D:
    def __init__(self, x: Real, y: Real) -> None:
        ...
        """
        self._BASE_ERRORS: Final[Sequence[TypeError]] = [
            TypeError("x must be a real number"),
            TypeError("y must be a real number"),
        ]
        self._ERRORS: Final[Sequence[TypeError | None]] = [
            self._BASE_ERRORS[0] if not isinstance(x, _INSTANCE_CHECK_REAL) else None,
            self._BASE_ERRORS[1] if not isinstance(y, _INSTANCE_CHECK_REAL) else None,
        ]
        exceptions: list[TypeError] = []
        for i in range(len(self._ERRORS)):
            if isinstance(self._ERRORS[i], TypeError):
                exceptions.append(cast(TypeError, self._ERRORS[i]))
        if len(exceptions) > 0:
    ...
    def x(self, value: Real) -> None:
        if not isinstance(value, _INSTANCE_CHECK_REAL):
            raise self._BASE_ERRORS[0]
        self._x = value

    @y.setter
    def y(self, value: Real) -> None:
        if not isinstance(value, _INSTANCE_CHECK_REAL):
            raise self._BASE_ERRORS[1]
        self._y = value
